 ...
 * ============================================================ */
    ...
    window.ensureTransactionsLoaded = async function(force) {
      ...
      // Soft path after local save: keep in-memory list (already includes new row) — avoid wipe
      if (window.__preferMemoryTx && same && window.__txCacheLoaded && !force) {
      ...
      try {
        ...
        // Union by id: IDB range result + in-memory rows still in range
        // (prevents "only new tx visible" when IDB read races or snapshot set a partial list)
        const byId = new Map();
        ...
        // Dirty ids that might not be in range result yet
        try {
        ...
        // Guard: never replace a richer in-memory list with a much smaller IDB result
        // (incomplete cursor / race). Keep the larger set unless force + explicit wipe.
        const merged = Array.from(byId.values());
        const memInRange = mem.filter(function(t) {
          if (!t || !t.date) return false;
          if (start && t.date < start) return false;
          if (end && t.date > end) return false;
          return true;
        });
        if (!force && memInRange.length > merged.length) {
          console.info('[STone] keep richer memory txs', memInRange.length, '>', merged.length);
          const richer = new Map();
          memInRange.forEach(function(t) { if (t && t.id) richer.set(String(t.id), t); });
          merged.forEach(function(t) { if (t && t.id && !richer.has(String(t.id))) richer.set(String(t.id), t); });
          window.appData.transactions = Array.from(richer.values());
        } else {
      ...
      try {
        ...
        if (n > 0 && SomtumStore.getAllTx) {
          ...
          try {
            ...
            console.info('[hydrate] ensureTransactionsLoaded soft-skip:', loadErr && loadErr.message ? loadErr.message : loadErr);
          }
        }
        ...
        console.warn('async hydrate failed (fallback to sync)', e);
